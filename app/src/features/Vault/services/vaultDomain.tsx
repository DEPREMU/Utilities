...
class VaultDomainService {
  ...
  public getFunctions = (): VaultFunctions => {
    return {
      getCurrentFolderId: () => {
        return (
      ...
      unlock: async (callback) => {
        ...
        if (!directory) {
          await new Promise<void>((resolve) => {
            const handlePressSelectFolder = async (
              errorInDefault?: boolean,
            ) => {
              if (errorInDefault) {
                alerts.showAlert(
                  "error",
        ...
        if (Object.keys(password).length === 0) {
          const success = await this.getFunctions().selectPassword();
          if (!success) {
      ...
      encryptFiles: async () => {
        ...
        if (!password) {
          this.getFunctions().selectPassword();
          return;
        }
        ...
        if (REPLACERS.isWeb) {
          ...
          if (success) {
            this.#setSlice("files", []);
            this.getFunctions().unlockFolder();
          } else {
            alerts.showAlert(
              "error",
              tTyped("vault.encryptFilesErrorMessage", {
          ...
          await Promise.all(
            files.map(async (file) => {
              try {
                ...
                if (!result) return;
                const res = await this.#dbService.saveEncFile(
                  folderId,
                  outpath,
                  {
      ...
      unlockFolder: async () => {
        ...
        if (REPLACERS.isWeb) {
          const files = await windowModule.loadEncryptedFiles(
            folderId,
            password,
          );
      ...
      renameFile: (file, newName) => {
        ...
        const nextOriginalUri = getEncryptedUriByName(
          file.originalUri,
          newName,
        );
